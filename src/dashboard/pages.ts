...
function footer(): string {
  ...
  <footer class="footer">
    <div class="container">
      ...
      Also building <a href="https://flatline.dev">Flatline</a> — cron/heartbeat monitoring for the same kind of side project.
    </div>
  </footer>`;
}
...
export function landingPage(host: string): string {
  ...
  <section class="hero">
    <div class="container">
      ...
      <h1>Your links deserve better<br/>than a <em>gray box</em></h1>
      <p class="hero-sub">
        SnapOG turns a title into a shareable Open Graph image — one API call,
        cached at the edge, no renderer to build and no server to maintain.
        Paste a URL into a <code style="font-family:var(--font-mono);font-size:0.9em;color:var(--teal);">&lt;meta&gt;</code> tag. That's the whole integration.
      </p>
      <div class="hero-cta">
        <a href="/register" class="btn btn-primary" style="font-size:15px;padding:12px 28px;">Get Free API Key</a>
        <a href="/#how-it-works" class="btn btn-ghost" style="font-size:15px;padding:12px 28px;">See it work ↓</a>
      </div>
      <p style="font-size:13px;color:var(--text-3);margin-top:20px;font-family:var(--font-mono);">Free tier, no credit card. 100 images/month, forever.</p>
  ...
  <section class="section" id="how-it-works">
    <div class="container">
      ...
      <h2 class="section-h2">Stop writing your own image renderer</h2>
      <p class="section-sub">
        If you've looked into this before, you've probably found <code>@vercel/og</code> or raw Satori —
        and found out that "free" library comes with a project attached: write the JSX layout,
        source and embed fonts, deploy a function, build your own cache. SnapOG is that project,
        already built, already running, already cached. Send a GET request. Get a PNG back. Done.
      </p>
...
<span class="c-key">&lt;meta</span> <span class="c-val">name=</span><span class="c-str">"twitter:image"</span>   <span class="c-val">content=</span><span class="c-str">"https://${host}/og?title=My+Post+Title&amp;key=YOUR_KEY"</span> <span class="c-key">/&gt;</span></pre>
  ...
  <section class="section" id="pricing">
    <div class="container">
      ...
      <div class="callout">
        <strong>One honest note</strong>, because we'd rather tell you now than have you find out later:
        cached responses count toward your monthly limit the same as freshly rendered ones. That's what
        keeps pricing simple and 1:1 with requests — call SnapOG once per piece of content (it's a URL
        in a meta tag, after all), not on every page render.
      </div>
    </div>
  </section>
  ...
  <section class="section" style="padding-top:0;">
    <div class="container">
      ...
      <div class="faq-list">
        <div class="faq-item">
          <p class="faq-q">What size image do I get back?</p>
          <p class="faq-a">1200×630 PNG — the standard size for Open Graph and Twitter summary cards, so it drops straight into existing meta tags without cropping surprises.</p>
        </div>
        <div class="faq-item">
          <p class="faq-q">How is this different from just self-hosting <code>@vercel/og</code>?</p>
          <p class="faq-a"><code>@vercel/og</code> is a rendering library — you still write the layout, embed fonts, deploy a function, and build caching around it. SnapOG is that finished, hosted, cached service: three templates, a <code>GET</code> request, a PNG back. You trade a project for a URL.</p>
        </div>
        <div class="faq-item">
          <p class="faq-q">Can I customize the design — my own fonts, logo, layout?</p>
          <p class="faq-a">Not today. SnapOG ships three fixed templates (<code>default</code>, <code>blog</code>, <code>article</code>) in dark or light, driven by <code>title</code>, <code>description</code>, <code>author</code>, <code>tag</code>, and <code>domain</code>. That's the tradeoff for zero setup — nothing to configure means nothing to get wrong.</p>
        </div>
        <div class="faq-item">
          <p class="faq-q">What happens when I hit my monthly limit?</p>
          <p class="faq-a">The API returns a <code>429</code> with your current tier and limit, plus an upgrade link. Nothing breaks silently — your existing images already embedded in pages keep working via cache until you're back under your limit or you upgrade.</p>
        </div>
        <div class="faq-item">
          <p class="faq-q">How do I get an API key?</p>
          <p class="faq-a">Register with just an email at <code>/register</code> — no credit card for the free tier. Upgrading to Pro or Business happens from your dashboard through Stripe checkout once you have a free key.</p>
        </div>
      </div>
    </div>
  </section>
  ...
  <section class="section">
    <div class="container" style="max-width:480px;">
      ...
      ${tier === 'pro' || tier === 'business'
        ? `<div class="alert" style="margin-bottom:20px;">We'll issue you a free-tier key now — after registering, upgrade to Pro/Business from your dashboard.</div>`
        : ''}
...
export function dashboardPage(key: ApiKey, recentCount: number, rawKey: string): string {
  ...
  <div class="container">
    <div class="dash-layout">
          ...
          ${
            key.tier === 'free'
              ? `<div style="margin-top:20px;padding-top:20px;border-top:1px solid var(--border);">
                   ...
                   <a href="/checkout?tier=pro&amp;key=${rawKey}" class="btn btn-primary" style="margin-top:10px;">Upgrade to Pro — $19/mo →</a>
                 </div>`
              : ''
          }
        </div>
      ...
      <span class="c-val">content=</span><span class="c-str">"https://snapog.dev/og?title=YOUR_TITLE&amp;key=${rawKey}"</span> <span class="c-key">/&gt;</span></pre>
          </div>
          <div class="code-block" style="margin-top:12px;">
            ...
            <pre><span class="c-key">curl</span> <span class="c-str">"https://snapog.dev/og?title=My+Blog+Post&amp;domain=myblog.com&amp;key=${rawKey}"</span> \
  <span class="c-val">--output</span> og.png && <span class="c-key">open</span> og.png</pre>
          </div>
        </div>
